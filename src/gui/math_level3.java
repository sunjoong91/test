...
import gui.math_level3.MyKeyListener;
import miro.Miro;
import util.UtilMath;

public class math_level3 extends JFrame {
	// 말 위치
	private int chrX = 0;
	private int chrY = 0;
	 ...
	 * Create the frame.
	 */
	public math_level3() throws Exception{
		setTitle("Math");
		...
		// 미로판 생성
		Miro miro = new Miro(3);

		getMiroList = miro.getMiro(3);
		getWordList = miro.getWordList(3,4);//정답수 1, 오답수 3
		...
		String sql = "select seq from TB_EDU_DATA WHERE LEVEL = 3 AND TYPE = 3";
		...
		Map<String,Object> map = utilMath.getMathProblemInfo(word, 3);
			...
			case 10:
				...
				for(int i =0; i<goList.size(); i++){
					...
					if((blankArr[1] + "," + blankArr[0]).equals(goList.get(goList.size()-1).get("go"))){
            			...
            			return;
            		}else if(Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]) > 4){
            			chrX = Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]);
						chrY = Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]);
						repaint();
						JOptionPane.showMessageDialog(null, "Y좌표 경로 이탈.");
            			...
            			return;
            		}else if(Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]) > 4){
            			chrX = Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]);
						chrY = Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]);
						repaint();
						JOptionPane.showMessageDialog(null, "X좌표 경로 이탈.");
...
//						repaint();
	...
	public void paint(Graphics g) {
		...
		// 미로 단어 font
		Font font = new Font("font", Font.BOLD, 45);
		// 단어 font
		Font font2 = new Font("font", Font.BOLD, 45);
		int startX = 6;
		int startY = 2;

		// 미로판
		for (int x = 0; x < 5; x++) {// level 1,2->4, level 3,4->5, level 5->6
			for (int y = 0; y < 5; y++) {// level 1,2->4, level 3,4->5, level 5->6
