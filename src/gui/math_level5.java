...
import gui.math_level5.MyKeyListener;
import miro.Miro;
import util.UtilMath;

public class math_level5 extends JFrame {
	// 말 위치
	private int chrX = 0;
	private int chrY = 0;
	 ...
	 * Create the frame.
	 */
	public math_level5() throws Exception{
		setTitle("Math");
		...
		//미로판 생성
		Miro miro = new Miro(5);//레벨 5

		getMiroList = miro.getMiro(5);//레벨 5
		getWordList = miro.getWordList(5,5);//5개 단어 뿌려지는 칸
		...
		String sql = "select seq from TB_EDU_DATA WHERE LEVEL = 5 AND TYPE = 3";
		...
		Map<String,Object> map = utilMath.getMathProblemInfo(word, 4);
            ...
            case 10:
				...
				for(int i =0; i<goList.size(); i++){
					...
					if((blankArr[1] + "," + blankArr[0]).equals(goList.get(goList.size()-1).get("go"))){
            			...
            			return;
            		}else if(Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]) > 5){
            			chrX = Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]);
						chrY = Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]);
						repaint();
						JOptionPane.showMessageDialog(null, "Y좌표 경로 이탈.");
            			...
            			return;
            		}else if(Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]) > 5){
            			chrX = Integer.parseInt(goList.get(i).get("go").toString().split(",")[0]);
						chrY = Integer.parseInt(goList.get(i).get("go").toString().split(",")[1]);
						repaint();
						JOptionPane.showMessageDialog(null, "X좌표 경로 이탈.");
