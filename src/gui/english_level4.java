...
public class english_level4 extends JFrame {
	// 말 위치
	private int chrX = 0;
	private int chrY = 0;
	...
	public english_level4() {
		...
		getWordList = miro.getWordList(4,6);//6개 단어 뿌려지는 칸
		...
		String sql = "select seq from TB_EDU_DATA WHERE LEVEL = 4 AND TYPE = 2";
		...
		Util util = new Util();
		Map<String,Object> map = util.randomWordCutEng(word, blankCnt);//blank 단어갯수 
		cuttingWordIndex = (List<Integer>) map.get("cuttingWordIndex");
		garbageWordList = (List<String>) map.get("garbageWordList");
		
		//System.out.println("cuttingWordIndex" + cuttingWordIndex.toString());
